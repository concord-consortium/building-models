...
| 2025-06-23 22:35:00 | Status Update | Done | InProgress | Reopened: Add undo/redo logging for node position updates | Chad |
| 2025-06-23 23:10:00 | Status Update | InProgress | Done | Undo/redo for node position updates implemented and verified | AI Agent |

> **Note:** Task re-opened to ensure that node position updates (x/y) are properly registered in the undo/redo stack, matching the behavior for other node property changes.
...
##### Completion Details (Undo/Redo for Node Position Updates)

**Technical Solution:**
- Refactored `handleUpdateNode` in `sage-api.ts` so that:
  - Property changes (title, initialValue, etc.) use `GraphStore.changeNode` (undoable)
  - Position changes (x/y) use `GraphStore.moveNodeCompleted` (undoable)
  - If both are present, both actions are performed, resulting in two undoable actions (matches UI behavior)
- No changes needed to the test interface; it already supports property-only, position-only, and combined updates.

**Verification:**
- Tested property-only, position-only, and combined updates via dev/api-test.html
- Confirmed each is undoable via the SageModeler UI controls
- Confirmed behavior matches standard SageModeler UI actions

> Task complete. See dev/api-test.html for test scenarios and logs.