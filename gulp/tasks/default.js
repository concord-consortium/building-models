...
gulp.task('watch', function() {
    ...
    gulp.watch(config.browserify.wireframe.watch, ['browserify-wireframe']);
    gulp.watch(config.browserify.globals.watch,   ['browserify-globals']);
    gulp.watch(config.assets.watch,               ['assets']);
    gulp.watch(config.vendor.watch,               ['vendor']);
});

gulp.task('build-all', ['coffeelint', 'browserify-app', 'browserify-wireframe', 'browserify-globals', 'css', 'assets', 'vendor']);
